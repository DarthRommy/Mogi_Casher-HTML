...
## 模擬部全体のシステム
### 注文&集計
1. Google Formで生徒のチケット注文を受け付け
2. Spreadsheetを整理して個人ごとの小計&チケット枚数の一覧にする
3. GASを利用して封筒にうまいこと切り分けて貼り付けられるように空欄を入れてフォーマット

というスタイルをとった。
### バーコードの割り当て
1. チケットの種類(全5種類)ごとにバーコードを一つ用意
2. バーコードは適当に乱数で作る(最後の一桁はチェックデジットにするけど)
3. どうでもいいけどEAN8桁で作りました

2020年はチケットの枚数だけバーコードを用意するとかいう鬼畜なことが行われていたらしい
### レジソフト
1. 純粋なhtml
2. チケットの名前、バーコード、単価をもとにしたCSVファイルを使って売り上げを記録する
3. Vercelで公開(あくまでプレビューとしてやけど)
## デザインテンプレート
